...
// 随机函数,来自：https://thebookofshaders.com/
...
void main(){
    ...
    // 增加渐变强度
    //    float strength = v_uv.x * 10.0;
    //    gl_FragColor = vec4(strength, strength, strength, 1.0);

    // 通过取模达到反复渐变效果
    //    float strength = mod(v_uv.x * 10.0, 1.0);       // mod是取模，等于JS 的 %
    //    gl_FragColor = vec4(strength, strength, strength, 1.0);

    // 利用step实现 斑马效果
    //    float strength = mod(v_uv.x * 10.0, 1.0);       // mod是取模，等于JS 的 %
    //    strength = step(0.5, strength);                 // step 是取边界值，小于0.5就是0，否则就是1

    // X轴和Y轴 都有
    //    float strengthX = mod(v_uv.x * 10.0, 1.0);       // mod是取模，等于JS 的 %
    //    strengthX = step(0.5, strengthX);                // step 是取边界值，小于0.5就是0，否则就是1
    //    float strengthY = mod(v_uv.y * 10.0, 1.0);       // mod是取模，等于JS 的 %
    //    strengthY = step(0.5, strengthY);                // step 是取边界值，小于0.5就是0，否则就是1
    //    float strength = strengthX + strengthY;

    // 引入u_time 进行移动
    //    float strength = mod((v_uv.x + u_time * 0.1) * 10.0, 1.0);       // mod是取模，等于JS 的 %
    //    strength = step(0.5, strength);

    // 方块图形
    //    float strength = step(0.2, mod(v_uv.x * 10.0, 1.0));
    //    strength *= step(0.2, mod(v_uv.y * 10.0, 1.0));
    ...
    // 22条纹相乘，实现渐变格子
    //    float strength = floor(v_uv.x*10.0)/10.0*floor(v_uv.y*10.0)/10.0;
    //    gl_FragColor =vec4(strength, strength, strength, 1);
    ...
    // 27根据相除，实现星星
    //    float strength = 0.15 / distance(v_uv, vec2(0.5, 0.5)) - 1.0;
    //    gl_FragColor =vec4(strength, strength, strength, strength);

    // 28设置vUv水平或者竖直变量
    //    float strength =0.15 / distance(vec2(v_uv.x, (v_uv.y-0.5)*5.0), vec2(0.5, 0.5)) - 1.0;
    //    gl_FragColor =vec4(strength, strength, strength, strength);
    ...
    //    vec2 rotateUv = rotate(v_uv, -u_time*5.0, vec2(0.5, 0.5));
    ...
    // 43 通过时间实现动态选择
    //    vec2 rotateUv = rotate(v_uv, -u_time*5.0, vec2(0.5));
    //    float alpha =  1.0 - step(0.5, distance(v_uv, vec2(0.5)));
    ...
    //    float strength = noise(v_uv * 20.0);
    //    gl_FragColor =vec4(strength, strength, strength, 1);

    //    float strength = step(0.5, noise(v_uv * 100.0));
    //    gl_FragColor =vec4(strength, strength, strength, 1);

    // 通过时间设置波形
    //    float strength = step(u_scale, cnoise(v_uv * 10.0 + u_time));
    //    gl_FragColor =vec4(strength, strength, strength, 1);
    ...
    // 波纹效果
    //    float strength = sin(cnoise(v_uv * 10.0)*5.0+u_time);
    //    gl_FragColor =vec4(strength, strength, strength, 1);
